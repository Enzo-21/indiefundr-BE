...
export type InvestmentMaturedWaitingEmailProps = {
  username?: string;
  fundName?: string;
  amountUsdt?: number;
...
export function InvestmentMaturedWaitingEmail({
  username = "",
  fundName = "your fund",
  amountUsdt = 0,
  ...
  portfolioUrl = "",
  logoUrl = "",
}: InvestmentMaturedWaitingEmailProps) {
  const amountLabel = amountUsdt.toFixed(2);
  const payoutLabel = projectedPayoutUsdt.toFixed(2);
  const preview = `Your ${fundName} investment reached its term — payout pending`;
...
export default InvestmentMaturedWaitingEmail;
