...
import { startPasswordlessAuth } from "@/services/auth/passwordless";
...
export async function POST(request: Request) {
  ...
  const result = await startPasswordlessAuth(body.data.email);
  if (!result.ok) {
  ...
  return Response.json({ email: result.email, isNewUser: result.isNewUser });
}
